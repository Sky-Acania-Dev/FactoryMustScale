...
namespace FactoryMustScale.Authoring
{
    ...
    /// <summary>
    /// Authoring asset for one factory layer state id.
    ///
    /// Mental model for designers:
    /// - Slots describe where items may enter/exit.
    /// - Recipes describe item/resource transforms.
    /// - Duration controls cycle time.
    /// - Progress payload channel selects where runtime progress is tracked.
    ///
    /// Runtime note:
    /// - This ScriptableObject is editor-only authoring data.
    /// - Simulation should consume only the baked <see cref="FactoryCellProcessData"/>.
    /// </summary>
    public sealed class FactoryCellProcessDefinition : ScriptableObject
    {
        ...
        {
            // Bake authoring fields into plain runtime structs/arrays so simulation can run
            // without referencing ScriptableObjects.
        ...
        private static FactoryProcessRecipeData[] BakeRecipes(FactoryProcessRecipeDefinition[] definitions)
        {
            ...
            for (int i = 0; i < definitions.Length; i++)
            {
                // Recipe arrays are baked into fixed dimensions to keep deterministic shape:
                // inputs=3, outputs=2, resource deltas=4.
                // Missing entries are zero-padded; extra entries are truncated.
